...
export async function providerHealth() {
  const remoteUrl = process.env.TRIBE_INFERENCE_URL || "";
  ...
  return [
    {
      ...
      available: true,
      active: !remoteActive,
      detail: "Deterministic serverless provider for Vercel deployments."
    },
    {
...
async function predictBrain(asset) {
  ...
  if (remoteUrl) {
    try {
      ...
      if (process.env.STIMLI_BRAIN_PROVIDER === "tribe-remote") {
        throw error;
      }
    }
  }
