...
const planCatalog = [
  {
    id: "research",
    name: "Research",
    asset_limit_per_hour: Number(process.env.STIMLI_RESEARCH_ASSET_LIMIT_PER_HOUR || 40),
    comparison_limit_per_hour: Number(process.env.STIMLI_RESEARCH_COMPARISON_LIMIT_PER_HOUR || 12),
    commercial: false,
    price_env: null
  },
  {
    id: "growth",
    name: "Growth",
    asset_limit_per_hour: Number(process.env.STIMLI_GROWTH_ASSET_LIMIT_PER_HOUR || 300),
    comparison_limit_per_hour: Number(process.env.STIMLI_GROWTH_COMPARISON_LIMIT_PER_HOUR || 100),
    commercial: true,
  ...
  },
  {
    id: "scale",
    name: "Scale",
    asset_limit_per_hour: Number(process.env.STIMLI_SCALE_ASSET_LIMIT_PER_HOUR || 2000),
    comparison_limit_per_hour: Number(process.env.STIMLI_SCALE_COMPARISON_LIMIT_PER_HOUR || 500),
    commercial: true,
...
export async function billingStatus(team = null) {
  const currentPlanId = normalizePlan(team?.plan);
  ...
  return {
    ...
    plans: planCatalog.map((plan) => ({
      ...plan,
      configured: plan.id === "research" || Boolean(plan.price_env && process.env[plan.price_env])
    }))
  };
}
...
export async function usageLimitsForWorkspace(workspaceId) {
  ...
  const plan = planById(normalizePlan(team?.plan));
  return {
...
export async function createCheckoutSession(request, team, requestedPlan) {
  ...
  if (!plan.commercial || !plan.price_env || !process.env[plan.price_env]) {
  ...
  const session = await stripe.checkout.sessions.create({
    mode: "subscription",
    customer: customerId,
    line_items: [{ price: process.env[plan.price_env], quantity: 1 }],
...
function billingConfigured() {
  return Boolean(process.env.STRIPE_SECRET_KEY && process.env.STIMLI_APP_URL);
}

function commercialUseEnabled() {
  const provider = String(process.env.STIMLI_BRAIN_PROVIDER || "fixture").toLowerCase();
  const usesTribe = provider.includes("tribe");
  return !usesTribe || process.env.STIMLI_TRIBE_COMMERCIAL_LICENSE === "1";
}

function licenseStatus() {
  return {
    provider: process.env.STIMLI_BRAIN_PROVIDER || "fixture",
    tribe_commercial_license: process.env.STIMLI_TRIBE_COMMERCIAL_LICENSE === "1",
    mode: commercialUseEnabled() ? "commercial-ready" : "research-only"
  };
}

function planById(planId) {
  return planCatalog.find((plan) => plan.id === normalizePlan(planId)) || planCatalog[0];
}
...
function normalizePlan(planId) {
  return planCatalog.some((plan) => plan.id === planId) ? planId : "research";
}
...
function appOrigin(request) {
  const configured = process.env.STIMLI_APP_URL;
  if (configured) {
    return configured.replace(/\/$/, "");
  }
  const host = header(request, "x-forwarded-host") || header(request, "host") || "stimli.vercel.app";
